...
console.log("Order Transac", orderTransaction)
  ...
  const handleReceiptsClick = async (row) => {
    try {
      ...
      await printerInstance.printColumnsText(
        ["Trans. Date:", formatDate(`${row.date_checkout}`)],
        [1, 1],
        [0, 2],
        [26, 26],
        576
      ...
      for (const product of productDetails) {
        ...
        const name = product.cart_specification_variants.length > 0 
          ? `${product.product_inventory.product.name} (${product.cart_specification_variants.map(variant => variant.specification_variant.variant_name).join(", ")})` : product.product_inventory.product.name
        await printerInstance.printColumnsText(
          [
            `${product.quantity}`,
            `${name}`,
      ...
      <div className="inventory-container">
        <div className="pos-head-container">
          ...
          <div className="filter-button-container">
            <div className="filter-button-container-container">
              ...
              {showDropdown && (
                <Dropdown.Menu ref={dropdownRef} show className="dropdown-menu">
                      ...
                      <div className="date-range">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            handleFilterIndicatorRange(e.target.value, endDate);
                          }}
                        />
        ...
        {isLoading ? (
          ...
          <div className="custom-card er-card">
            <div className="inventory-stock-container d-flex m-2">
              <h2 className="mb-0 me-3">Sales Invoice Report</h2>
                ...
                <table className="custom-user-table">
                  ...
                  <tbody>
                    {productDetailsCheckout.map((prod, i) => (
                      <tr key={i}>
                        <td>
                          {prod.product_inventory.product.sku}
                        </td>
                        <td>
                          <div className="d-flex flex-column p-0">
                            <span>
                              {
                                prod.product_inventory.product
                                  .name
                              }
                              <br/>

                          {prod.cart_specification_variants.length > 0 
                            ? `(${prod.cart_specification_variants.map(variant => variant.specification_variant.variant_name).join(", ")})` : null}
                            </span>
                        ...
                        {prod.cart_specification_variants.length > 0 
                            ? `+ ${prod.cart_specification_variants.map(variant => variant.specification_variant.variant_price).join(", ")}` : null}
                        </td>
                        <td>{prod.quantity}</td>
                        <td>
        ...
        <div className="paper-container my-3 pt-0">
          <div className="receipt receipt-eighty">
            <div className={`receipt-title-name  cust-bold`}>
              <p className="cust-text text-dark mb-1">DUAL TECH</p>
            </div>
              ...
              {receiptProductData.map((order) => (
                <>
                  <div className="receipt-orders">
                    ...
                    <p className="mb-0">
                      {order.product_inventory.product.name}<br/>

                      {order.cart_specification_variants.length > 0 
          ? `(${order.cart_specification_variants.map(variant => variant.specification_variant.variant_name).join(", ")})` : null}
                    </p>
                    <p className="mb-0">
