...
const BulkLoadReports = ({ authrztn }) => {
  ...
  useEffect(() => {
    ...
    setFilteredTransactions(filteredData);
  }, [fromDate, endDate, bulkHistoryData, selectedUser]);
  ...
  const bulkDetailColumn = [
    ...
    {
      name: "TOP UP ",
      selector: (row) => row.load_transaction.load_amount,
    },
    {
      name: "NEW BALANCE",
    ...
    try {
      ...
      if (dataForExport.length > 0) {
        if (type == "Excel") {
            ...
            dataForExport
              .map((data) => {
                ...
                return `${data.bulk_load.transaction_number_id},${
                  ...
                  data.load_transaction.student_balance.balance
          ...
          link.setAttribute("download", "bulk_load_reports.csv");
          ...
          link.href = url;
          link.download = "bulk_load_reports.pdf"; // Set the desired filename here
          link.click();
          window.open(url);
      ...
      >
        <Modal.Header>
          <div className="d-flex w-100 p-3 justify-content-between align-items-center">
            <div>
              <p className="h2">Bulk Upload Details</p>
            </div>
