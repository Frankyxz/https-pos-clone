...
const InventoryReports = ({ authrztn }) => {
  ...
  const receivedColumns = [
    ...
    },
    {
      name: "SKU #",
      selector: (row) =>
        row.product_inventory_accumulate.product_inventory
          .product.sku,
    },
    {
      name: "STOCK NAME",
  ...
  const outboundColumns = [
    ...
    },
    {
      name: "SKU #",
      selector: (row) =>
        row.product_inventory_outbound.product_inventory
          .product.sku,
    },
    {
      name: "STOCK NAME",
  ...
  const stockColumns = [
    ...
    },
    {
      name: "SKU #",
      selector: (row) =>
        row.product_inventory_counting.product_inventory
          .product.sku,
    },
    {
      name: "STOCK NAME",
  ...
  const handleExport = async (format) => {
    ...
    if (value === 0) {
      ...
      dataToExport = inventoryReportsData;
      exportDataFunc = () => {
        if (format === "excel") {
          const exportedData = inventoryReportsData.map((item) => ({
            "SKU #": item.product.sku,
            "STOCK NAME": item.product.name,
            QUANTITY: !filter ? item.quantity : item.product.product_inventories.length > 0
            ? item.product.product_inventories[0].quantity
            : 0,
            STATUS: item.status,
          }));
          ...
          const tableData = inventoryReportsData.map((item) => {
            ...
            let quantity = !filter ? item.quantity : item.product.product_inventories.length > 0
            ? item.product.product_inventories[0].quantity : 0

            if (quantity === 0) {
              status = "No Stock";
            } else if (
              quantity < item.product.threshold
            ) {
              status = "Low Stock";
            } else {
              status = "In Stock";
      ...
      exportDataFunc = () => {
        if (format === "excel") {
          const exportedData = filteredReceivedData.map((item) => ({
            ...
            "SKU #":
              item.product_inventory_accumulate.product_inventory
                .product.sku,
            "STOCK NAME":
              item.product_inventory_accumulate.product_inventory
                .product.name,
      ...
      exportDataFunc = () => {
        if (format === "excel") {
          const exportedData = filteredOutboundData.map((item) => ({
            ...
            "SKU #":
              item.product_inventory_outbound.product_inventory
                .product.sku,
            "STOCK NAME":
              item.product_inventory_outbound.product_inventory
                .product.name,
      ...
      exportDataFunc = () => {
        if (format === "excel") {
          const exportedData = filteredCountingData.map((item) => ({
            ...
            "SKU #":
              item.product_inventory_counting.product_inventory
                .product.sku,
            "STOCK NAME":
              item.product_inventory_counting.product_inventory
                .product.name,
