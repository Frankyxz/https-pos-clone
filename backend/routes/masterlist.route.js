...
router.route("/supervisor").get(async (req, res) => {
  try {
    const data = await MasterList.findAll({
      ...
      where: {
        user_type: {
          [Op.notIn]: ["Superadmin", "Cashier"], // Exclude Superadmin and Cashier
        },
        col_status: {
          [Op.ne]: "Archive",
        },
...
//function sa pagfind ng manual input RFID number
router.route("/checkRFIDEmployee").post(async (req, res) => {
  try {
    const { inputtedRFID } = req.body;

    const checkRFIDNumber = await MasterList.findOne({
      where: {
        rfid: inputtedRFID,
      },
    });
    ...
    if (checkRFIDNumber) {
      const { user_type } = checkRFIDNumber;
      ...
      if (user_type === "Cashier") {
        ...
        res.status(200).json({ data: checkRFIDNumber });
      } else {
...
router.route("/checkpin").post(async (req, res) => {
  try {
    ...
    const cartQuantities = findCart.map((orderedCart) => orderedCart.quantity);
    const prodInvId = findCart.map(
      (orderedCart) => orderedCart.product_inventory_id
    );
    const cartIds = findCart.map((orderedCart) => orderedCart.transaction_id);
    ...
    res.status(200).json({ message: "Void Transaction Success" });
  } catch (err) {
    console.error(err);
    res.status(500).json("Error");
  }
